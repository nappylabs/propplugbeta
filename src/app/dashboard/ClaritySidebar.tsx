...
export const ClaritySidebar = () => {
  ...
  return (
    <motion.aside 
      ...
      className="w-full lg:w-96 bg-white border-l border-zinc-200 p-8 flex flex-col gap-8 h-full rounded-l-[2.5rem] shadow-xl"
    >
      {/* Clarity Meter */}
      <div className="space-y-3">
        <div className="flex justify-between items-end">
          ...
          <span className={`text-2xl font-black transition-colors ${state.clarityScore === 100 ? 'text-emerald-500' : 'text-orange-600'}`}>
            {state.clarityScore}%
          </span>
        </div>
        <div className="h-3 w-full bg-zinc-100 rounded-full overflow-hidden">
          <motion.div 
            ...
            className={`h-full ${state.clarityScore === 100 ? 'bg-emerald-500' : 'bg-orange-600'}`}
          />
        </div>
      ...
      <div className="space-y-6">
        <h3 className="text-2xl font-black text-zinc-900 tracking-tight leading-tight">
          Capture Your <br/><span className="text-orange-600">"Aha" Moment</span>
        </h3>
        <div className="space-y-4">
          <label className="text-xs font-bold text-zinc-500 uppercase">Why this property?</label>
          <textarea 
            onChange={(e) => calculateClarity(e.target.value)}
            placeholder="e.g. The rental yield covers the bond, and the area is gentrifying..."
            className="w-full h-40 p-5 bg-zinc-50 border-2 border-transparent focus:border-orange-100 rounded-2xl focus:ring-0 focus:bg-white focus:shadow-lg transition-all font-medium text-zinc-700 resize-none placeholder:text-zinc-300"
          />
        </div>
      </div>

      {/* Endowment Hook */}
      <div className="mt-auto p-6 bg-orange-50 rounded-3xl border border-orange-100 relative overflow-hidden">
        <Sparkles className="absolute top-4 right-4 text-orange-200" size={40} />
        <p className="text-xs text-orange-900 font-bold leading-relaxed italic relative z-10">
