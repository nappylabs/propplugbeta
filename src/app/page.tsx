...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-zinc-900 selection:bg-orange-100 scroll-smooth">
      ...
      <nav className="max-w-6xl mx-auto mt-6 px-6 py-3 flex justify-between items-center bg-white/90 backdrop-blur-xl sticky top-6 z-[100] border border-slate-200 rounded-full shadow-xl">
        ...
        <div className="flex items-center gap-6">
          <a href="#demo" className="text-sm font-bold text-slate-600 hover:text-orange-600 transition-colors hidden md:block">Demo</a>
          <a href="#pricing" className="text-sm font-bold text-slate-600 hover:text-orange-600 transition-colors hidden md:block">Pricing</a>
          <div className="h-4 w-[1px] bg-slate-200 hidden md:block"></div>
          <Link href="/login" className="text-sm font-bold text-slate-600 hover:text-orange-600 transition-colors">
            Login
          </Link>
      ...
      <header className="max-w-6xl mx-auto px-6 pt-20 pb-24 grid lg:grid-cols-2 gap-12 items-center">
        <div className="space-y-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-orange-50 text-orange-700 rounded-full text-xs font-black uppercase tracking-widest border border-orange-100">
            <Zap size={14} className="animate-pulse" /> Stop Guessing, Start Winning
          </div>
          <h1 className="text-5xl md:text-7xl font-black leading-[0.95] tracking-tighter text-zinc-900">
            Kill Analysis Paralysis. <br />
            Find That <span className="text-orange-600 inline-block hover:animate-vibrate hover:scale-105 transition-transform cursor-help">10/10 Deal</span> Fast.
          </h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <a href="https://chrome.google.com/webstore" target="_blank" className="px-8 py-4 bg-white text-zinc-900 border-2 border-slate-200 rounded-3xl font-bold text-lg hover:border-zinc-900 hover:-translate-y-1 transition-all flex items-center justify-center gap-2">
              Install Extension <Zap size={20} />
            </a>
            <Link href="/signup" className="px-8 py-4 bg-zinc-900 text-white rounded-3xl font-bold text-lg hover:-translate-y-1 hover:shadow-card-hover transition-all flex items-center justify-center gap-2 group">
              Start Free 5-day Trial <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />
            </Link>
          </div>
        </div>
        <div className="relative group">
          <div className="absolute -inset-10 bg-gradient-to-tr from-orange-500/20 to-red-500/20 blur-3xl rounded-full"></div>
      ...
      <section id="demo" className="py-24 bg-white">
        <div className="max-w-6xl mx-auto px-6">
          <div className="mb-16 text-zinc-900">
            ...
            <div className="lg:col-span-5 space-y-4">
              {benefits.map((b, i) => (
                <button 
                  ...
                  className={`w-full text-left p-8 rounded-[2rem] transition-all flex items-start gap-6 border-2 ${activeBenefit === i ? 'bg-orange-50 border-orange-600 shadow-lg' : 'bg-transparent border-transparent hover:bg-slate-50'}`}
                >
                  <div className={`p-3 rounded-2xl ${activeBenefit === i ? 'bg-orange-600 text-white' : 'bg-white text-slate-400 border border-slate-200'}`}>
                    {b.icon}
                  </div>
                  <div>
                    <h4 className="font-black text-lg mb-1">{b.title}</h4>
                    <p className={`text-sm leading-relaxed ${activeBenefit === i ? 'text-orange-900/70 font-medium' : 'text-slate-400'}`}>{b.desc}</p>
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="max-w-6xl mx-auto px-6">
            ...
            <div className="flex justify-center gap-4 bg-white p-2 rounded-full w-fit mx-auto shadow-sm border border-slate-200">
              {Object.keys(profiles).map((p) => (
                <button 
                  ...
                  className={`px-8 py-3 rounded-full font-bold text-sm transition-all ${activeProfile === p ? 'bg-zinc-900 text-white shadow-lg' : 'text-slate-500 hover:text-orange-600'}`}
                >
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center bg-white p-12 rounded-[4rem] shadow-xl border border-slate-100">
            <div className="space-y-8">
              <h3 className="text-4xl font-black text-orange-600 tracking-tighter">{profiles[activeProfile as keyof typeof profiles].title}</h3>
      ...
      <section className="py-24 bg-white overflow-hidden">
        <div className="max-w-5xl mx-auto px-6">
          <div className="text-center mb-20">
            <h2 className="text-5xl font-black text-zinc-900 tracking-tighter">Screen and build your <br/>deal book in seconds.</h2>
          </div>
      ...
      <section id="pricing" className="py-24 bg-slate-50">
        <div className="max-w-5xl mx-auto px-6 text-center">
          ...
          <div className="flex items-center justify-center gap-6 mb-16 bg-white p-3 rounded-full w-fit mx-auto shadow-sm border border-slate-200">
            <span className={`text-sm font-bold ${!isAnnual ? 'text-orange-600' : 'text-slate-400'}`}>Monthly</span>
            ...
            <span className={`text-sm font-bold ${isAnnual ? 'text-orange-600' : 'text-slate-400'}`}>Annual <span className="text-[10px] text-emerald-600 ml-1 font-black">-40%</span></span>
          </div>
      ...
      <footer className="bg-zinc-900 text-white py-20">
        <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-4 gap-12">
          ...
          <div>
            <h4 className="font-black mb-6 uppercase tracking-widest text-xs text-orange-400">Product</h4>
          ...
          <div>
            <h4 className="font-black mb-6 uppercase tracking-widest text-xs text-orange-400">Company</h4>
            <ul className="space-y-4 text-slate-400 text-sm font-bold">
          ...
          <div className="bg-zinc-800/50 p-8 rounded-[2.5rem] border border-zinc-700">
            ...
            <div className="flex gap-2">
              <input type="email" placeholder="Email" className="bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-2 text-sm w-full outline-none focus:border-orange-500 transition-all" />
              <button className="bg-orange-600 p-2 rounded-xl hover:bg-orange-700 transition-all"><ArrowRight size={18} /></button>
            </div>
          </div>
        </div>
        <div className="max-w-6xl mx-auto px-6 mt-20 pt-8 border-t border-zinc-800 text-center text-slate-500 text-xs font-bold">
  ...
  return (
    <div className={`flex flex-col ${reverse ? 'lg:flex-row-reverse' : 'lg:flex-row'} items-center gap-24 group`}>
      <div className="flex-1 text-left space-y-6">
        <span className="text-8xl font-black text-slate-100 group-hover:text-orange-50 transition-colors">{number}</span>
        <h3 className="text-4xl font-black text-zinc-900 tracking-tighter">{title}</h3>
...
function PriceCard({ name, price, period = '', subtext, features, cta, popular }: any) {
  return (
    <div className={`p-12 rounded-[3.5rem] text-left border-2 transition-all hover:-translate-y-4 flex flex-col h-full ${popular ? 'border-orange-600 bg-white shadow-card-hover relative' : 'border-slate-100 bg-slate-50'}`}>
      {popular && <span className="absolute -top-5 left-1/2 -translate-x-1/2 bg-orange-600 text-white px-6 py-2 rounded-full text-xs font-black uppercase tracking-widest">Growth Plan</span>}
      ...
      <div className="mb-10">
        ...
        {subtext && <p className="text-xs text-orange-600 font-bold mt-1">{subtext}</p>}
      </div>
      ...
      <Link href="/signup" className={`block w-full py-6 rounded-3xl font-black text-xl text-center transition-all ${popular ? 'bg-orange-600 text-white shadow-lg hover:bg-orange-700' : 'bg-zinc-900 text-white hover:bg-zinc-800'}`}>
        {cta}
      </Link>
    </div>
  );
