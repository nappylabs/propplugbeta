// 4. Insights Drawer Component
// File: src/components/insights/InsightsDrawer.tsx
...
const InsightCard: React.FC<InsightCardProps> = ({ prompt, response, onSave, onPin }) => {
  const [isEditing, setIsEditing] = useState(!response?.completedAt);
  ...
  if (isEditing) {
    return (
      <div className="bg-zinc-800/50 border border-orange-500/50 rounded-2xl p-6 animate-fade-in">
        <h4 className="font-bold text-white mb-4">{prompt.question}</h4>
        <textarea
          value={text}
          onChange={(e) => setText(e.target.value)}
          className="w-full bg-zinc-900 border border-zinc-700 rounded-lg p-3 text-white text-sm resize-none focus:outline-none focus:border-orange-500"
          rows={4}
          placeholder="Your thoughts..."
        />
        <div className="mt-4">
          <h5 className="text-xs text-zinc-400 mb-2">Quick Suggestions:</h5>
          <div className="flex flex-wrap gap-2">
            {prompt.chips.map(chip => (
              <button
                key={chip}
                onClick={() => toggleTag(chip)}
                className={`px-3 py-1 text-xs font-bold rounded-full border transition-colors ${selectedTags.includes(chip) ? 'bg-orange-500 border-orange-500 text-white' : 'bg-zinc-700 border-zinc-700 text-zinc-300 hover:border-zinc-500'}`}
              >
                {chip}
              </button>
            ))}
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <button onClick={handleSave} className="px-4 py-2 text-xs font-bold bg-orange-600 text-white rounded-lg hover:bg-orange-700">Save Insight</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-zinc-800/30 border border-zinc-800 rounded-2xl p-6 transition-all hover:border-zinc-700 group">
      <div className="flex justify-between items-start">
        <h4 className={`font-bold ${response?.completedAt ? 'text-white' : 'text-zinc-400'}`}>{prompt.question}</h4>
        <div className="flex items-center gap-3">
            <button onClick={() => onPin(prompt.id)} className={`text-zinc-600 hover:text-amber-400 transition-colors ${response?.isPinned ? 'text-amber-400' : ''}`}>
                <Star size={14} fill={response?.isPinned ? 'currentColor' : 'none'} />
            </button>
            {response?.completedAt ? (
            <button onClick={() => setIsEditing(true)} className="text-zinc-400 hover:text-white"><Edit size={14} /></button>
            ) : (
            <button onClick={() => setIsEditing(true)} className="flex items-center gap-1 text-orange-400 text-xs font-bold hover:text-orange-300">
      ...
      {response?.completedAt && (
        <div className="mt-3 text-sm text-zinc-300 flex items-start gap-2">
          <CheckCircle size={16} className="text-emerald-500 mt-0.5 shrink-0" />
          <div>
            <p>{response?.response}</p>
            <div className="flex flex-wrap gap-2 mt-2">
              {response?.tags?.map(tag => (
                <span key={tag} className="px-2 py-0.5 text-[10px] font-bold bg-zinc-700 text-zinc-300 rounded-full">{tag}</span>
              ))}
            </div>
          </div>
        </div>
...
export const InsightsDrawer: React.FC<InsightsDrawerProps> = ({ isOpen, onClose, insights, onUpdateInsight, onPinInsight, onUpdateFreeform, persona }) => {
  ...
  const pinnedInsights = useMemo(() => {
    if (!insights) return [];
    return Object.values(insights.prompts).filter(p => p.isPinned && p.completedAt);
  }, [insights]);
  ...
  return (
    <div className="fixed inset-0 z-40 flex justify-end">
      ...
      <div className="relative w-full max-w-2xl h-full bg-zinc-900 border-l border-zinc-800 shadow-2xl flex flex-col animate-slide-in-right">
        {/* Header */}
          ...
          <div className="flex space-x-2">
            {['all', 'key_factors', 'risks', 'opportunities', 'personal_fit', 'actions'].map(tab => (
              <button
                ...
                className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-colors ${activeTab === tab ? 'bg-zinc-700 text-white' : 'text-zinc-400 hover:bg-zinc-800'}`}
              >
        ...
        <div className="flex-grow p-6 overflow-y-auto space-y-4">
          ...
          {pinnedInsights.length > 0 && (
            <div className="bg-zinc-900/50 border border-zinc-700 rounded-2xl p-6 mb-6">
                <h3 className="text-sm font-bold text-amber-400 flex items-center gap-2 mb-4"><Star size={14} /> Key Takeaways</h3>
                <ul className="space-y-3">
                    {pinnedInsights.map(p => (
                        <li key={p.id} className="text-sm text-zinc-300 border-l-2 border-zinc-700 pl-3">
                            <p className="font-bold text-white">{p.question}</p>
          ...
          <div className="bg-zinc-800/30 border border-zinc-800 rounded-2xl p-6">
            ...
            <textarea
              defaultValue={insights?.freeformNotes || ''}
