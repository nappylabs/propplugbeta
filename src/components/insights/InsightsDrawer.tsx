...
const InsightCard: React.FC<InsightCardProps> = ({ prompt, response, onSave, onPin }) => {
  ...
  if (isEditing) {
    return (
      <div className="bg-slate-800/50 border border-indigo-500/50 rounded-2xl p-6 animate-fade-in">
  ...
  return (
    <div className="bg-slate-800/30 border border-slate-800 rounded-2xl p-6 transition-all hover:border-slate-700 group">
      <div className="flex justify-between items-start">
        <h4 className={`font-bold ${response?.completedAt ? 'text-white' : 'text-slate-400'}`}>{prompt.question}</h4>
        <div className="flex items-center gap-3">
            <button onClick={() => onPin(prompt.id)} className={`text-slate-600 hover:text-amber-400 transition-colors ${response?.isPinned ? 'text-amber-400' : ''}`}>
